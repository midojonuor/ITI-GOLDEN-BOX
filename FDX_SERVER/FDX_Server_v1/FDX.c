...
static Std_ReturnType DataExchangeHandler(uint16_t *Datasize, uint8_t *inbuffer, uint16_t *groupId)
{
	...
	FDX_DataExchange_t *pFDX_DataExchange = (FDX_DataExchange_t *)&inbuffer[CMD_SIZE_OFFSET];
 ...
 */
Std_ReturnType FDX_CreateStartCmd(uint8_t *outputBuffer)
{
	Std_ReturnType errorStatus = E_OK;
	FDX_Start_t *pFDX_Start = (FDX_Start_t *)&outputBuffer[CMD_SIZE_OFFSET];
...
/**
 * @brief                 This Function creates data exchange command
 * 
 * @param outputBuffer    Return the created frame
 ...
 */
Std_ReturnType FDX_CreateDataExchangeCmd(uint8_t  *outputBuffer, uint16_t groupID,
		uint16_t dataSize, uint8_t  *dataBytes)

{
	Std_ReturnType errorStatus = E_OK;
	FDX_DataExchange_t *pFDX_DataExchange = (FDX_DataExchange_t *) &outputBuffer[CMD_SIZE_OFFSET];


	if(pFDX_DataExchange != NULL)
	{
		/* Assign command code, command size,GroupID and Data size */
		pFDX_DataExchange->CommandSize			= CMD_DATA_EXCHANGE_SIZE + dataSize;
 ...
 */
Std_ReturnType FDX_CreateDataRequestCmd(uint8_t *outputBuffer, uint16_t seqNum, uint16_t groupID)
{
	Std_ReturnType errorStatus = E_OK;
	FDX_DataRequest_t *pFDX_DataRequest = (FDX_DataRequest_t *)&outputBuffer[CMD_SIZE_OFFSET];
 ...
 * 
 * @param buffer          Return status command frame
 * @param seqNum          Holds the frame order
 * @param status          Holds the frame state 
 * @param timeStamp       Holds the estimated action time
 * @return Std_ReturnType returns E_OK if there is a data request command and E_Not_OK if there is no data request command
 */
Std_ReturnType FDX_CreateStatusCmd(uint8_t *outputBuffer, uint16_t status, uint64_t timeStamp)
{
	Std_ReturnType errorStatus = E_OK;
	FDX_Status_t *pFDX_Status = (FDX_Status_t *)&outputBuffer[CMD_SIZE_OFFSET];
 ...
 */
Std_ReturnType FDX_CreateFrameHeader(uint8_t *outputBuffer, uint16_t seqNum, uint16_t numOfCmd)
{

	Std_ReturnType errorStatus = E_OK;
	FDX_Header_t *pFDX_Header = (FDX_Header_t *)outputBuffer;
	...
	if(pFDX_Header != NULL)
	{
		...
		pFDX_Header->NumOfCMD 		= (uint16_t) numOfCmd;
