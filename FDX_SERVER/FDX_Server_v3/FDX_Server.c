...
#define PWM_INChannels_NUM		2u
#define PWM_OUTChannels_NUM		2u
...
static uint8_t ADCChannelTable[ADC_CHANNEL_NUM] = {
		...
		if(GET_DIR(groupId) == DIR_APP_TO_GOLDENBOX)
		{
			...
			case PERIPH_ID_PWM_INPUT:
				...
				for(indx = 0; indx < pFDX_DataExchange->DataSize; indx += 3)
				{
					...
					fflush(stdout);
#endif
					if(server->recv_msg[DATAEX_BYTES_OFFSET + indx] >= PWM_INChannels_NUM)
					{
						ApiStatus = E_NOT_OK;
						break;
